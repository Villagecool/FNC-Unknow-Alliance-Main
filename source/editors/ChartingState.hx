...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		\nZ/X - Zoom in/out
		\nHJKL/VBNM - Place Notes While Playing
		\n
		...
		if (!blockInput)
		{
			...
			if(FlxG.sound.music.playing){
				...
				if(FlxG.keys.justPressed.V)
					addNote(getStrumTime(Math.floor((strumLine.y + strumLine.height) / GRID_SIZE) * GRID_SIZE) + sectionStartTime(), 0 + (_song.notes[curSection].mustHitSection ? 0 : 4));
	
				if(FlxG.keys.justPressed.B)
					addNote(getStrumTime(Math.floor((strumLine.y + strumLine.height) / GRID_SIZE) * GRID_SIZE) + sectionStartTime(), 1 + (_song.notes[curSection].mustHitSection ? 0 : 4));
	
				if(FlxG.keys.justPressed.N)
					addNote(getStrumTime(Math.floor((strumLine.y + strumLine.height) / GRID_SIZE) * GRID_SIZE) + sectionStartTime(), 2 + (_song.notes[curSection].mustHitSection ? 0 : 4));
	
				if(FlxG.keys.justPressed.M)
					addNote(getStrumTime(Math.floor((strumLine.y + strumLine.height) / GRID_SIZE) * GRID_SIZE) + sectionStartTime(), 3 + (_song.notes[curSection].mustHitSection ? 0 : 4));
